scoreboard objectives add bss.used_as minecraft.used:minecraft.armor_stand {"translate":"bss.scoreboard.used_armor_stand"}
scoreboard objectives add bss.gen.cobblestone_tick dummy
scoreboard objectives add bss.gen.iron_tick dummy
scoreboard objectives add bss.gen.gold_tick dummy
scoreboard objectives add bss.gen.copper_tick dummy
scoreboard objectives add bss.dropped_item dummy
scoreboard objectives add bss.used_map minecraft.used:minecraft.map {"translate":"bss.scoreboard.used_map"}
scoreboard objectives add bss.mana_tick dummy
scoreboard objectives add bss.mana dummy
scoreboard objectives add bss.max_mana dummy
scoreboard objectives add bss.mana_usage dummy
scoreboard objectives add bss.used_gapple minecraft.used:minecraft:golden_apple
scoreboard objectives add bss.used_enchgapple minecraft.used:minecraft:enchanted_golden_apple
